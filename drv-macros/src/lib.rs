...
/// `PartialEq + Clone + Debug + Default`. Fields can be annotated with
...
/// Mark a function as a memoized expression.
///
...
pub fn expr(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    match expr::expand(item) {
        Ok(tokens) => tokens.into(),
        Err(err) => err.to_compile_error().into(),
    }
}

/// Collect all atoms, lenses, and exprs declared in this crate and generate
/// the cache structs.
///
/// Must appear once, after all `#[drv::atom]`, `#[drv::lens]`, and `#[drv::expr]`
/// declarations.
#[proc_macro]
